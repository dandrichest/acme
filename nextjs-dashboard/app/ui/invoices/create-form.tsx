...
import { useActionState } from 'react'; // If using React 18, switch to: useFormState from 'react-dom'
...
export default function EditInvoiceForm({
  ...
  // Bind the id (first parameter). React will supply (prevState, formData).
  const updateInvoiceWithId = updateInvoice.bind(null, invoice.id);
  const [state, formAction] = useActionState(updateInvoiceWithId, initialState);
  // React 18 alternative:
  // const [state, formAction] = useFormState(updateInvoiceWithId, initialState);

  // If your DB stores cents, convert for display
  const displayAmount =
    typeof invoice.amount === 'number' ? (invoice.amount / 100).toString() : '';
